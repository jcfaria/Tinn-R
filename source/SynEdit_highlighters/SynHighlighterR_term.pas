...
$Id: SynHighlighterR.pas, v 1.19.0.0 2013/09/04 15:30:00 JCFaria $
...
type
  TSynR_termSyn = class(TSynCustomHighLighter)

  private
    //fStringStarter: WideChar;  // used only for rsMultilineString stuff   // Multiline is not suitable for Rterm!
...
function TSynR_termSyn.IsIdentChar(AChar: WideChar): Boolean;
begin
  case AChar of
...
function TSynR_termSyn.GetKeywordIdentifiers: TUnicodeStringList;
const
...
function TSynR_termSyn.IdentKind(MayBe: PWideChar): TtkTokenKind;
var
  i:    Integer;
  temp: PWideChar;
  s:    UnicodeString;
...
constructor TSynR_termSyn.Create(AOwner: TComponent);
begin
...
destructor TSynR_termSyn.Destroy;
begin
  FKeywords.Free;
  inherited;
end;

procedure TSynR_termSyn.SymbolProc;
begin
  Inc(Run);
  fTokenID:= tkSymbol;
end;

procedure TSynR_termSyn.OperatorProc;
begin
  Inc(Run);
  fTokenID:= tkOperator;
end;

procedure TSynR_termSyn.CRProc;
begin
  fTokenID:= tkSpace;
  case FLine[Run + 1] of
    #10: Inc(Run, 2);
...
procedure TSynR_termSyn.CommentProc;
begin
{
  fTokenID:= tkComment;
  Inc(Run);
...
procedure TSynR_termSyn.IdentProc;
begin
...
procedure TSynR_termSyn.LFProc;
begin
  fTokenID:= tkSpace;
  Inc(Run);
end;

procedure TSynR_termSyn.NullProc;
begin
  fTokenID:= tkNull;
  Inc(Run);
end;
...
{
procedure TSynR_termSyn.NumberProc;
type
  TNumberState =
    (
    nsStart,
...
procedure TSynR_termSyn.SpaceProc;
begin
  Inc(Run);
  fTokenID:= tkSpace;
...
procedure TSynR_termSyn.String34Proc;
...
procedure TSynR_termSyn.String39Proc;
begin
{
  fTokenID:= tkString;
  ...
  repeat
    case FLine[Run] of
      #0, #10, #13 : begin
        //fStringStarter:= #39;
        //fRange:= rsMultilineString;  // Multiline is not suitable for Rterm!
        Break;
      end;
      else
        Inc(Run);
...
//Is Multiline is not suitable for Rterm?
(**
procedure TSynR_termSyn.StringEndProc(EndChar: WideChar);
begin
...
procedure TSynR_termSyn.UnknownProc;
begin
  Inc(Run);
  fTokenID:= tkUnknown;
end;

procedure TSynR_termSyn.Next;
begin
  fTokenPos:= Run;
...
function TSynR_termSyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynR_termSyn.GetEol: Boolean;
begin
  Result:= Run = fLineLen + 1;
end;

function TSynR_termSyn.GetRange: Pointer;
begin
  Result:= Pointer(fRange);
end;

function TSynR_termSyn.GetTokenID: TtkTokenKind;
begin
  Result:= fTokenId;
end;

function TSynR_termSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynR_termSyn.GetTokenKind: integer;
begin
  Result:= Ord(fTokenId);
end;

procedure TSynR_termSyn.ResetRange;
begin
  fRange:= rsUnknown;
end;

procedure TSynR_termSyn.SetRange(Value: Pointer);
begin
  fRange:= TRangeState(Value);
end;

{
function TSynR_termSyn.IsFilterStored: Boolean;
begin
  Result:= fDefaultFilter <> SYNS_FilterR;
end;
}

class function TSynR_termSyn.GetLanguageName: string;
begin
  Result:= SYNS_LangRterm;
end;


class function TSynR_termSyn.GetFriendlyLanguageName: UnicodeString;
begin
  Result:= SYNS_FriendlyLangR_term;
end;

initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynR_termSyn);
{$ENDIF}
finalization
  GlobalKeywords.Free;
end.
