...
const
  KeyWords_URI: array[0..15] of UnicodeString = (
...
function TSynRSyn.HashKey(Str: PWideChar): Integer;
begin
  Result := 0;
...
function TSynRSyn.IsCurrentToken(const Token: UnicodeString): Boolean;
var
  I: Integer;
  Temp: PWideChar;
begin
...
procedure TSynRSyn.ProtocolProc;
var
  Key: Integer;
...
function TSynRSyn.AltFunc(Key: Integer): TtkTokenKind;
begin
  Result := tkUnknown;
end;

procedure TSynRSyn.SetAlreadyVisitedURIFunc(Value: TAlreadyVisitedURIFunc);
begin
  fAlreadyVisitedURI := Value;
end;

procedure TSynRSyn.SetURIAttri(const Value: TSynHighlighterAttributes);
begin
  fURIAttri.Assign(Value);
end;

procedure TSynRSyn.SetVisitedURIAttri(const Value: TSynHighlighterAttributes);
begin
  fVisitedURIAttri.Assign(Value);
end;

function TSynRSyn.FuncFtp(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkFtpLink
  else
    Result := tkUnknown;
end;

function TSynRSyn.FuncGopher(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkGopherLink
  else
...
function TSynRSyn.FuncHttp(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkHttpLink
  else
...
function TSynRSyn.FuncHttps(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkHttpsLink
  else
...
function TSynRSyn.FuncMailto(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkMailtoLink
  else
...
function TSynRSyn.FuncNews(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkNewsLink
  else
...
function TSynRSyn.FuncNntp(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkNntpLink
  else
...
function TSynRSyn.FuncProspero(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
...
function TSynRSyn.FuncTelnet(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkTelnetLink
  else
...
function TSynRSyn.FuncWais(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidURI then
    Result := tkWaisLink
  else
...
function TSynRSyn.FuncWeb(Key: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords_URI[Key]) and IsValidWebLink then
    Result := tkWebLink
  else
    Result := tkUnknown;
end;

function TSynRSyn.IsUnreserved(AChar: WideChar): Boolean;
begin
  Result := IsAlphaNum(AChar) or IsMark(AChar);
end;

function TSynRSyn.IsAlphaNum(AChar: WideChar): Boolean;
begin
  Result := SynIsCharAlphaNumeric(AChar);
end;

function TSynRSyn.IsMark(AChar: WideChar): Boolean;
begin
  case AChar of
    '-', '_', '.', '!', '~', '*', '''', '(' , ')':
      Result := True;
...
function TSynRSyn.IsReserved(AChar: WideChar): Boolean;
begin
  case AChar of
    ';', '/', '?', ':', '@', '&', '=', '+', '$', ',', '%', '#':
      Result := True;
...
function TSynRSyn.IsURIChar(AChar: WideChar): Boolean;
begin
  Result := IsReserved(AChar) or IsUnreserved(AChar);
end;

function TSynRSyn.IsNeverAtEMailAddressEnd(AChar: WideChar): Boolean;
begin
  Result := (AChar = '.') or (AChar = '@');
end;

function TSynRSyn.IsEMailAddressChar(AChar: WideChar): Boolean;
begin
  case AChar of
    '.', '_', '-', '@':
      Result := True;
...
function TSynRSyn.IsNeverAtEnd(AChar: WideChar): Boolean;
begin
  Result := (IsMark(AChar) and (AChar <> '''')) or
            (IsReserved(AChar) and (AChar <> '/') and (AChar <> '$'));
end;

function TSynRSyn.IsValidEmailAddress: Boolean;
var
...
function TSynRSyn.IsValidURI: Boolean;
var
...
function TSynRSyn.IsValidWebLink: Boolean;
var
