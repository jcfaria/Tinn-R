...
begin
  ...
  with frmTinnMain do begin
    with dlgODMain do begin
      if IsFile1 then InitialDir := LastOpenedFolder1
      else InitialDir := LastOpenedFolder2;
      FileName := '';
      Filter:= dlgSDMain.Filter;  //SaveDialog was altered by JCFaria
...
begin
  frmTinnMain.dlgSDMain.InitialDir := extractfilepath(fn1);
  //JCFaria
  //frmTinnMain.dlgSDMain.Filter := 'Text files (*.txt)|*.txt';
  //frmTinnMain.dlgSDMain.DefaultExt := 'txt';
  frmTinnMain.dlgSDMain.FilterIndex := 3;
  if not frmTinnMain.dlgSDMain.execute then exit;
  ...
  try
    ...
    //JCFaria
    with frmTinnMain do begin
      fName := dlgSDMain.FileName;
      fileExtension := lowerCase(ExtractFileExt(fName));
      if not (fileExtension = frmTinnMain.SaveAsFileExt) then
        fName := dlgSDMain.FileName + frmTinnMain.SaveAsFileExt;
    end;

    if FileExists(fName) then
      if MessageDlg(fName + #13 + #13 +
                    'Ok to overwrite this file?',
