...
type
  ...
  private
    ...
    procedure pDrawCharacter(const CharValue: Integer; const CharText: string; const HorizMult, VertMult: Integer);
    procedure GetFonts;
...
begin
  ...
  BaseNum := 0;
  FontSize := 8;
  FontName := frmMain.coEditor.Font.Name;
...
begin
  ...
  if BaseNum = 0 then begin
    pDrawCharacter(0, 'NUL', HorizMult, VertMult); // Ctrl @, NULL
    pDrawCharacter(1, 'SOH', HorizMult, VertMult); // Ctrl A, Start of Header
    pDrawCharacter(2, 'STX', HorizMult, VertMult); // Ctrl B,Start of Text
    pDrawCharacter(3, 'ETX', HorizMult, VertMult); // Ctrl C,end of Text
    pDrawCharacter(4, 'EOT', HorizMult, VertMult); // Ctrl D,end of Transmission
    pDrawCharacter(5, 'ENQ', HorizMult, VertMult); // Ctrl E,Enquiry
    pDrawCharacter(6, 'ACK', HorizMult, VertMult); // Ctrl F,Acknowlodge
    pDrawCharacter(7, 'BEL', HorizMult, VertMult); // Ctrl G,Bell
    pDrawCharacter(8, 'BS', HorizMult, VertMult); // Ctrl H,Backspace
    pDrawCharacter(9, 'TAB', HorizMult, VertMult); // Ctrl I,Horizontal Tab
    pDrawCharacter(10, 'LF', HorizMult, VertMult); // Ctrl J,Linefeed
    pDrawCharacter(11, 'VT', HorizMult, VertMult); // Ctrl K,Vertical Tab
    pDrawCharacter(12, 'FF', HorizMult, VertMult); // Ctrl L,Form Feed
    pDrawCharacter(13, 'CR', HorizMult, VertMult); // Ctrl M,Carridge Return
    pDrawCharacter(14, 'SO', HorizMult, VertMult); // Ctrl N,Shift Out
    pDrawCharacter(15, 'SI', HorizMult, VertMult); // Ctrl O,Shift in
    pDrawCharacter(16, 'DLE', HorizMult, VertMult); // Ctrl P,Delete
    pDrawCharacter(17, 'DC1', HorizMult, VertMult); // Ctrl Q,Device Control 1
    pDrawCharacter(18, 'DC2', HorizMult, VertMult); // Ctrl R,Device Control 2
    pDrawCharacter(19, 'DC3', HorizMult, VertMult); // Ctrl S,Device Control 3
    pDrawCharacter(20, 'DC4', HorizMult, VertMult); // Ctrl T,Device Control 4
    pDrawCharacter(21, 'NAK', HorizMult, VertMult); // Ctrl U,Negative Acknowledge
    pDrawCharacter(22, 'SYN', HorizMult, VertMult); // Ctrl V,Synchronise
    pDrawCharacter(23, 'ETB', HorizMult, VertMult); // Ctrl W,end Block ??
    pDrawCharacter(24, 'CAN', HorizMult, VertMult); // Ctrl X,Cancel
    pDrawCharacter(25, 'EM', HorizMult, VertMult); // Ctrl Y,end Message
    pDrawCharacter(26, 'SUB', HorizMult, VertMult); // Ctrl Z,Sub
    pDrawCharacter(27, 'ESC', HorizMult, VertMult); // Ctrl [,Escape
    pDrawCharacter(28, 'FS', HorizMult, VertMult); // Ctrl \,Form Separator
    pDrawCharacter(29, 'GS', HorizMult, VertMult); // Ctrl ],Group Separator
    pDrawCharacter(30, 'RS', HorizMult, VertMult); // Ctrl ^,Record Separator
    pDrawCharacter(31, 'US', HorizMult, VertMult); // Ctrl _,Unit Separator
  end;
  ...
  for I := Start to 127 do
    pDrawCharacter(I, Chr(BaseNum + I), HorizMult, VertMult);
...
procedure TfrmAsciiChart.pDrawCharacter(const CharValue: Integer; const CharText: string; const HorizMult, VertMult: Integer);
var
...
begin
  ...
  if Assigned(TfrmEditor(frmMain.MDIChildren[0])) then
    with TfrmEditor(frmMain.MDIChildren[0]).synEditor do
