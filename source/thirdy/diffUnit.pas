...
type
  ...
  private
    ...
    function RecursiveDiff2(bottom1, bottom2, top1, top2: integer): boolean;
  ...
  public
    ...
    {$ENDIF}
  published
    //Precision - valid range is 1 .. 5
    //1 == fastest but least precise; 5 == slowest but most precise
...
procedure TDiff.SetPrecision(precision: integer);
begin
  if precision < 1 then
    fPrecision := 1
...
begin
  ...
  try
    ...
    else if fPrecision = 5 then
      Result := RecursiveDiff(bottom1 -1, bottom2 -1, top1, top2)
    else
      Result := RecursiveDiff2(bottom1 -1, bottom2 -1, top1, top2);
