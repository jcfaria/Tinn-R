...
{$ENDIF}
    ...
    procedure DrawGroupings(const Rect: TRect; DataCol: Integer;
...
var
  IsShiftPressed: Boolean;  //jcfaria
  KeyState: TKeyboardState;  //jcfaria
begin
  if False
  {$IFDEF USE_BDE}
...
procedure WriteTitleText(ACanvas: TCanvas; ARect: TRect; DX, DY: Integer;
  const Text: string; Alignment: TAlignment; TextEllipsis: TSMTextEllipsis; IsWordBreak: Boolean {$IFDEF SMDelphi4} ; ARightToLeft: Boolean {$ENDIF});
const
...
{$ENDIF}
    DrawText(ACanvas.Handle, PChar(Text), Length(Text), ARect,
             ...
             {$IFDEF SMDelphi4} or RTL[ARightToLeft] {$ENDIF} );
  end
  else
  begin
...
//  PopUpMenu := FDBPopUpMenu;
  ...
  FExOptions := [eoENTERlikeTAB, eoKeepSelection, eoStandardPopup, eoBLOBEditor, eoTitleWordWrap];
  HScrollbar := True;
  VScrollbar := True;
//  ScrollBars := ssBoth;
//  Color := clInfoBk;
...
var
  ...
  ArrayOfColumnsLen: array {$IFDEF SMDelphi5} {$ELSE} [0..200] {$ENDIF} of Integer;
  StartPoint: Integer;
...
//        Font.Assign(TitleFont);
      ...
      begin
        ...
        begin
          ...
          if (Columns[i].Field.DataType = ftBoolean) and (eoBooleanAsCheckbox in ExOptions) then
            TempLen := 2
          else
...
var
  ...
  procedure CheckTab1(GoForward: Boolean);
  var
    ACol, Original: Integer;
  begin
    if (dgMultiSelect in Options) and DataLink.Active then
...
procedure TSMDBGrid.DrawGroupings(const Rect: TRect; DataCol: Integer;
  Column: TColumn; State: TGridDrawState);
var
  i{, intGroupIndex}: Integer;  //jcfaria
...
//        bmp.Free;
      ...
      DrawGroupings(Rect, DataCol, Column, State);
  ...
  if DefaultDrawing and
     (GridStyle.Style <> gsNormal) and
     not (eoShowLookup in ExOptions) then
    DefaultDrawColumnCell(Rect, DataCol, Column, State);
