...
procedure TSynBaseCompletionProposal.ExecuteEx(s: UnicodeString; x, y: integer; Kind : SynCompletionType);
  ...
  begin
    ...
    begin
      tmpY := tmpY - tmpHeight - (Form.CurrentEditor  as TCustomSynEdit).LineHeight - 1;
      if tmpY < 0 then
        tmpY := 0;
    end;
    ...
    if (tmpX + tmpWidth > Screen.WorkAreaWidth) then  // J.C.Faria
    begin
      tmpX:= GetWorkAreaWidth - tmpWidth - 5;  // small space buffer
      if tmpX < 0 then
        tmpX:= 0;
    ...
    if (tmpY + tmpHeight > Screen.WorkAreaHeight) then  // It is better to two monitors
    begin
      tmpY:= tmpY - tmpHeight - (Form.CurrentEditor  as TCustomSynEdit).LineHeight - 1;  // J.C.Faria
      if tmpY < 0 then
        tmpY:= 0;
    end;
    ...
    Form.Height:= tmpHeight;
    Form.Top   := tmpY;
    Form.Left  := tmpX + 5;  // J.C.Faria: it is better to view the cursor of the editor
  end;
