...
$Id: SynHighlighterAll.pas, v 1.19.0.0 2013/08/21 19:30:00 JCFaria Exp $
...
@lastmod(2013-08-19)
The SynHighlighterAll implements a highlighter for R for the SynEdit projects.
}

{$IFNDEF QSYNHIGHLIGHTERALL}
unit SynHighlighterAll;
{$ENDIF}
...
function TSynAllSyn.IsIdentChar(AChar: WideChar): Boolean;
begin
  case AChar of
...
function TSynAllSyn.IdentKind(MayBe: PWideChar): TtkTokenKind;
var
  i:    Integer;
  temp: PWideChar;
  s:    UnicodeString;
...
begin
  ...
  *)
  ...
  while IsIdentChar(temp^) or      // '_', '0'..'9', 'A'..'Z', 'a'..'z':
        (MayBe[i] = '.') do begin  // All: an keyword (Programing, Function, Dataset or Plotting) can has a point '.' inside
    Inc(temp);
    Inc(i);
  end;
  // ----------------------------------------------------------------------------------------------------------------------
...
constructor TSynAllSyn.Create(AOwner: TComponent);
begin
  ...
  fDefaultFilter:= SYNS_FilterAll;
end; { Create }

destructor TSynAllSyn.Destroy;
begin
  FKeywords.Free;
  inherited;
end;

procedure TSynAllSyn.SymbolProc;
begin
  inc(Run);
  fTokenID:= tkSymbol;
end;

procedure TSynAllSyn.OperatorProc;
begin
  inc(Run);
  fTokenID:= tkOperator;
end;

procedure TSynAllSyn.CRProc;
begin
  fTokenID:= tkSpace;
  case FLine[Run + 1] of
    #10: inc(Run, 2);
...
procedure TSynAllSyn.CommentProc;
begin
  fTokenID:= tkComment;
  inc(Run);
...
procedure TSynAllSyn.IdentProc;
begin
...
procedure TSynAllSyn.LFProc;
begin
  fTokenID:= tkSpace;
  inc(Run);
end;

procedure TSynAllSyn.NullProc;
begin
  fTokenID:= tkNull;
  inc(Run);
end;

procedure TSynAllSyn.NumberProc;
type
  TNumberState =
    (
    nsStart,
...
procedure TSynAllSyn.SpaceProc;
begin
  inc(Run);
  fTokenID:= tkSpace;
...
procedure TSynAllSyn.String34Proc;
begin
  fTokenID:= tkString;
...
procedure TSynAllSyn.String39Proc;
begin
  fTokenID:= tkString;
...
procedure TSynAllSyn.StringEndProc(EndChar: WideChar);
begin
...
procedure TSynAllSyn.UnknownProc;
begin
  inc(Run);
  fTokenID:= tkUnknown;
end;

procedure TSynAllSyn.Next;
begin
  fTokenPos:= Run;
...
function TSynAllSyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynAllSyn.GetEol: Boolean;
begin
  Result:= Run = fLineLen + 1;
end;

function TSynAllSyn.GetRange: Pointer;
begin
  Result:= Pointer(fRange);
end;

function TSynAllSyn.GetTokenID: TtkTokenKind;
begin
  Result:= fTokenId;
end;

function TSynAllSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynAllSyn.GetTokenKind: integer;
begin
  Result:= Ord(fTokenId);
end;

procedure TSynAllSyn.ResetRange;
begin
  fRange:= rsUnknown;
end;

procedure TSynAllSyn.SetRange(Value: Pointer);
begin
  fRange:= TRangeState(Value);
end;

function TSynAllSyn.IsFilterStored: Boolean;
begin
  Result:= fDefaultFilter <> SYNS_FilterAll;
end;

class function TSynAllSyn.GetLanguageName: string;
begin
  Result:= SYNS_LangAll;
end;

function TSynAllSyn.GetSampleSource: UnicodeString;
begin
  Result:=
    '''All sample'''                                +
...
begin
  Result:= SYNS_FriendlyLangAll;
end;

initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynAllSyn);
{$ENDIF}
finalization
end.
